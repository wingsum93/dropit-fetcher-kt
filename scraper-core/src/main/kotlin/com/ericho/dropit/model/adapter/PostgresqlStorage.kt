...
class PostgresqlStorage(private val config: DatabaseConfig) : Storage {
    ...
    private val rateLimitsTable = DSL.table("rate_limits")
    private val rateLimitNameField = DSL.field("name", String::class.java)
    private val rateLimitNextAvailableAtField = DSL.field("next_available_at", Timestamp::class.java)

    private val managedTables = listOf("products", "departments", "jobs", "syncs", "rate_limits")
    ...
    override fun findProducts(limit: Int): List<ProductEntity> {
        ...
        try {
            return dataSource.connection.use { connection ->
                ...
                ctx.selectFrom(productsTable)
                    .orderBy(
                        DSL.coalesce(productRemoteLastUpdateAtField, productCreatedAtField).desc(),
                        productIdField.desc()
                    )
                    .limit(limit)
                    .fetch { mapProduct(it) }
            ...
            fail("findProducts", limit.toString(), exception)
        }
    }
    ...
    override fun findJobsBySync(syncId: Int, status: JobStatus?, type: JobType?): List<JobEntity> {
        try {
            return dataSource.connection.use { connection ->
                val ctx = DSL.using(connection, SQLDialect.POSTGRES)
                var condition = jobSyncIdField.eq(syncId)
                if (status != null) {
                    condition = condition.and(jobStatusField.eq(status.name))
                }
                if (type != null) {
                    condition = condition.and(jobTypeField.eq(type.name))
                }
            ...
            fail("findJobsBySync", "$syncId:${status?.name}:${type?.name}", exception)
        }
    }
        ...
        try {
            return dataSource.connection.use { connection ->
                ...
                ctx.fetch(
                    ...
                    RETURNING id, sync_id, job_type, status, created_at, updated_at, dedupe_key
                    """.trimIndent(),
            ...
            fail("claimPendingJobs", limit.toString(), exception)
        }
    }
    ...
    override fun findSyncById(id: Int): SyncEntity? {
        try {
            return dataSource.connection.use { connection ->
                val ctx = DSL.using(connection, SQLDialect.POSTGRES)
                ctx.selectFrom(syncsTable)
                    .where(syncIdField.eq(id))
                    .fetchOne { mapSync(it) }
            }
        } catch (exception: Exception) {
            fail("findSyncById", id.toString(), exception)
        }
    }
    ...
    override fun updateSyncStatusFromJobs(syncId: Int): SyncEntity? {
        try {
            return dataSource.connection.use { connection ->
                val ctx = DSL.using(connection, SQLDialect.POSTGRES)
                val sync = ctx.selectFrom(syncsTable)
                    .where(syncIdField.eq(syncId))
                    .fetchOne { mapSync(it) }
                    ?: return null

                val activeCount = ctx.fetchOne(
                    """
                    SELECT COUNT(*)
                    FROM jobs
                    WHERE sync_id = ?
                    ...
                    JobStatus.PENDING.name,
                    JobStatus.IN_PROGRESS.name
                )?.get(0, Number::class.java)?.toInt() ?: 0
                if (activeCount > 0) return sync

                val totalCount = ctx.fetchOne(
                    "SELECT COUNT(*) FROM jobs WHERE sync_id = ?",
                    syncId
                )?.get(0, Number::class.java)?.toInt() ?: 0
                if (totalCount == 0) return sync

                val errorCount = ctx.fetchOne(
                    "SELECT COUNT(*) FROM jobs WHERE sync_id = ? AND status = ?",
                    syncId,
                    JobStatus.ERROR.name
                )?.get(0, Number::class.java)?.toInt() ?: 0

                val next = if (errorCount > 0) {
                    sync.copy(status = SyncStatus.RETRY, finishedAt = null)
                } else {
                    sync.copy(status = SyncStatus.DONE, finishedAt = Instant.now())
                }
                updateSyncEntity(next)
            }
        } catch (exception: Exception) {
            fail("updateSyncStatusFromJobs", syncId.toString(), exception)
        }
    }
        ...
        try {
            dataSource.connection.use { connection ->
                ...
                try {
                    val ctx = DSL.using(connection, SQLDialect.POSTGRES)
                    val now = Instant.now()
                    ctx.insertInto(rateLimitsTable)
                        .columns(rateLimitNameField, rateLimitNextAvailableAtField)
                        .values(name, Timestamp.from(now))
                        .onConflict(rateLimitNameField)
                        .doNothing()
                        .execute()

                    val current = ctx.select(rateLimitNextAvailableAtField)
                        .from(rateLimitsTable)
                        .where(rateLimitNameField.eq(name))
                        .forUpdate()
                        .fetchOne(rateLimitNextAvailableAtField)
                        ?.toInstant()
                        ?: now

                    val reservedAt = if (current.isAfter(now)) current else now
                    ctx.update(rateLimitsTable)
                        .set(rateLimitNextAvailableAtField, Timestamp.from(reservedAt.plusMillis(spacingMs)))
                        .where(rateLimitNameField.eq(name))
                        .execute()
            ...
            fail("reserveRateLimitSlot", name, exception)
        }
    }
    ...
    private fun resetAllManagedTablesIfSchemaStale(ctx: DSLContext) {
        ...
        ctx.execute("DROP TABLE IF EXISTS rate_limits CASCADE")
    }
    ...
    private fun ensureRateLimitsSchema(ctx: DSLContext) {
        val name = DSL.field("name", SQLDataType.VARCHAR(80).nullable(false))
        val nextAvailableAt = DSL.field("next_available_at", SQLDataType.TIMESTAMPWITHTIMEZONE.nullable(false))

        ctx.createTableIfNotExists(rateLimitsTable)
            .column(name)
            .column(nextAvailableAt)
            .constraints(DSL.primaryKey(name))
            .execute()
    }
